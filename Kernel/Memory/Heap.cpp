...
public:
    ...
    {
        // Default constructor
        Logger::Trace(__func__, "heap: Major: constructed new major: addr: 0x%p, pages: %zu, size: %zu", this, m_pages, m_size);
    }
...
public:
    ...
    {
        // Default constructor
        Logger::Trace(__func__, "heap: Minor: constructed new minor: addr: 0x%p, magic: 0x%08zX", this, m_magic);
    }
...
void init()
{
    if (!isMagicChosen) {
        ...
        Logger::Debug(__func__, "heap: OK: 0x%08zX DEAD: 0x%08zX", magicHeapOk, magicHeapDead);
    }
}
...
{
    Logger::Trace(__func__, "heap: allocateNewPage: size: %zu", size);
    ...
    Logger::Trace(__func__, "heap: allocateNewPage: major: 0x%p, pages: %zu totalAllocated: %zu", major, pages, totalAllocated);
    return major;
}

static inline void align(void** ptr)
{
    Logger::Trace(__func__, "heap: align: before: 0x%p", *ptr);
    if (ALIGNMENT > 1) {
    ...
    Logger::Trace(__func__, "heap: align: after: 0x%p", *ptr);
}

static inline void unalign(void** ptr)
{
    Logger::Trace(__func__, "heap: unalign: before: 0x%p", *ptr);
    if (ALIGNMENT > 1) {
...
{
    RAIIMutex raii(heapLock);
    Logger::Trace(__func__, "heap: malloc: requestedSize: %zu", requestedSize);
    ...
    if (size == 0) {
        Logger::Trace(__func__, "heap: malloc returning nullptr");
        return nullptr;
    }

    if (memoryList.IsEmpty()) {
        Logger::Trace(__func__, "heap: malloc: memory list is empty");
    ...
    while (major) {
        ...
        if (diff < size + sizeof(Minor)) {
            Logger::Trace(__func__, "heap: malloc: case 1");
            if (major->Next()) {
        ...
        if (major->llMinor.Head() == nullptr) {
            Logger::Trace(__func__, "heap: malloc: case 2");
            ...
            align(&ptr);
            Logger::Trace(__func__, "heap: malloc'd: 0x%p @ 2", ptr);
            return ptr;
        }

        // Case 3: Block is in use and there's enough space at the start of the block
        Logger::Trace(__func__, "heap: malloc: case 3");
        ...
        if (diff >= (size + sizeof(Minor))) {
            ...
            // Align the pointer to the nearest bounary (block headers may cause unalignment)
            align(&ptr);
            Logger::Trace(__func__, "heap: malloc'd: 0x%p @ 3", ptr);
            return ptr;
        }

        // Case 4: There is enough space in this block, but is it contiguous?
        Logger::Trace(__func__, "heap: malloc: case 4");
        Minor* minor = static_cast<Minor*>(major->llMinor.Head());
        // Loop within the block and check contiguity
        while (minor) {
            // Case 4.1: End of minor block in major block.
            if (minor->Next() == nullptr) {
                Logger::Trace(__func__, "heap: malloc: case 4.1");
                ...
                if (diff >= (size + sizeof(Minor))) {
                    ...
                    // Align the pointer to the nearest bounary (block headers may cause unalignment)
                    align(&ptr);
                    Logger::Trace(__func__, "heap: malloc'd: 0x%p @ 4.1", ptr);
                    return ptr;
                }
            }
            ...
            if (minor->Next()) {
                Logger::Trace(__func__, "heap: malloc: case 4.2");
                ...
                if (diff >= (size + sizeof(Minor))) {
                    ...
                    // Align the pointer to the nearest bounary (block headers may cause unalignment)
                    align(&ptr);
                    Logger::Trace(__func__, "heap: malloc'd: 0x%p @ 4.2", ptr);
                    return ptr;
                }
            }
        ...
        if (major->Next()) {
            Logger::Trace(__func__, "heap: malloc: case 5");
            if (startedBet) {
                major = static_cast<Major*>(memoryList.Head());
                startedBet = false;
                continue;
        ...
        Logger::Trace(__func__, "heap: malloc: getting major next");
        major = static_cast<Major*>(major->Next());
    }

    if (ptr) {
        Logger::Trace(__func__, "heap: malloc'd: 0x%p @ end", ptr);
    } else {
        Logger::Trace(__func__, "heap: malloc returning nullptr @ end");
    }
...
void free(void* ptr)
{
    RAIIMutex raii(heapLock);
    unalign(&ptr);
    Logger::Trace(__func__, "heap: free: freeing: 0x%p", ptr);
    if (ptr == nullptr) {
        return;
    }
    ...
    {
        Logger::Trace(__func__, "heap: free: minor: 0x%p, invalid magic 0x%08zX", minor, magic);
