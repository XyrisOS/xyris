...
// had to unfortunately abandon the idea. Fonts are a pain, so go check
// out the work that SkiftOS has done. They deserve it.
static uint8_t fontData[FONT_COUNT][FONT_WIDTH] = {
    ...
    for (int fy = 0; fy < 9; fy++) {
        for (int fx = 0; fx < 9; fx++) {
            if (fx != FONT_WIDTH && fy != FONT_HEIGHT && (fontData[fp][fy] & (1 << fx))) {
                ...
                // Not part of the glyph. Draw background color.
                pixel(x + fx, y + fy, back);
            }
        }
    }
}
